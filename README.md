...
* js 执行机制 Event Loop
        ...
        js是单线程的,代码从上往下执行,分为同步任务和异步任务.
        同步和异步分别进入不同的执行场所,同步任务进入主线程,异步任务进入Event Table,并且注册函数
        当指定事件完成时,Event Table 会将这个函数一如Event Queue
        当主线程内任务执行完毕时,会去Event Queue读取对应的函数,进入主线程执行
        上述过程会不断重复,成为 Event Loop
        ...
        事件循环的顺序，决定js代码的执行顺序。1....进入整体代码(宏任务)后，开始第一次循环。接着执行所有的微任务。2....然后再次从宏任务开始，找到其中一个任务队列执行完毕，再执行所有的微任务
